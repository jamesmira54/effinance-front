...
import { RankingStudentListingFormProps } from "./RankingStudentListing.types";
...
import { RankingListFormProps } from "../ranking-list/RankingList.types";

const RankedStudentsListingForm: React.FC<
{
    initialData: any, 
    ...
    const initialValues = useMemo<RankingStudentListingFormProps>(() => ({
        studentId: initialData.id,
        sponsorshipId: initialData.sponsorId,
        appStage: initialData.appStage || APPLICATION_STAGE.FINAS_PROPER,
        appStatus: initialData.appStatus || APPLICATION_STATUS.AWARDED,
    ...
    const submitHandler = async (values: RankingListFormProps, setSubmitting: (isSubmitting: boolean) => void) => {
        try {
            ...
            response = await ApplicationAPI.updateApplicationStatus(initialData.id, values);
            if(response) {
                setError(false);
                setErrorMessage('');
                onSuccess(response);
...
export default RankedStudentsListingForm;